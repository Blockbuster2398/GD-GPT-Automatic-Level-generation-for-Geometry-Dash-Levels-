...
class MultiHeadAttention(nn.Module): # Defines a class that inherits from nn.module, the class that defines all model
    def __init__(self, d_model, num_heads):
        ...
        assert d_model % num_heads == 0, "d_model must be divisible by num_heads" # Raises error if not fulfilled
...
class PositionWiseFeedForward(nn.Module):
    ...
    # Two Layer linear component with relu in between
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, max_seq_length):
        ...
        pe = torch.zeros(max_seq_length, d_model) # 0 Tensor
